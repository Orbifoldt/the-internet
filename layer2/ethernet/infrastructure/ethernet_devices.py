...
class EthernetSwitch(DeviceWithInterfaces):
    ...
    def receive(self, frame: EthernetFrame, interface_num: int, **kwargs) -> None:
        self.forward(frame, interface_num)
        self.update_cache(frame.source, interface_num)

    def forward(self, frame: EthernetFrame, port_num):
    ...
    def broadcast_to_all(self, frame: EthernetFrame, port_num):
...
class EthernetEndpoint(DeviceWithInterfaces):
    ...
    def receive(self, frame: EthernetFrame, **kwargs):
    ...
    def send(self, frame: EthernetFrame, **kwargs):
